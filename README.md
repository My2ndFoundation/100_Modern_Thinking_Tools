...
| `.claude/` | LLM (skills/state) | Claude Code 配置：见下 |
...
LLM 会：
1. 跑 diff: `find raw -maxdepth 1 -name '*.md'` 与 `.claude/state/ingested.tsv` 对比
2. 列出所有未处理的新文件
...
LLM 会按 [`CLAUDE.md` §5](./CLAUDE.md) 的 10 步流程走完：读原文 → alias 去重 → **checkpoint** → 写来源页 → 建/更实体页 → 更新 `index.md` → 写 `log.md` → 追加 `.claude/state/ingested.tsv` → 报告 → **不**自动 commit。
...
| 已处理状态 | `.claude/state/ingested.tsv` |
